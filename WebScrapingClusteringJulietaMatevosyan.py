...
class ArticleTopics:
    ...
    def get_topics(self):
        ...
        lda_model = LdaModel(corpus = corpus,
                        id2word = id2word,
                        num_topics = 3, # the purpose of topic modeling is to divide the document text into more than 1 topiics
                        random_state = 0, # find a parameter that divide the topics more clearly
                        chunksize = 100,
                        alpha = 'auto',
...
# Create an instance of ArticleTopics that can call the same methods (functions) defined inside the class

for document_text in article_df['article_text']:
    class_instance = ArticleTopics(document_text) # class with its input passed in
    topics = class_instance.get_topics()
    tokens = class_instance.sudachipy_tokenizer(document_text)
    print('Below are the tokens: \n', tokens)


class_instance.get_topics()

article_topics = {url:topic for url, topic in zip(article_df['article_url'], article_df['article_text'].apply(get_article_topics))}
